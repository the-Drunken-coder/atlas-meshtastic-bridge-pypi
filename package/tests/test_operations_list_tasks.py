...
@pytest.mark.asyncio
async def test_list_tasks_operation_forwards_offset_and_status() -> None:
    client = MagicMock()
    client.list_tasks = AsyncMock(return_value={"tasks": []})

    result = await list_tasks.run(
        client,
        envelope=None,
...
@pytest.mark.asyncio
async def test_list_tasks_operation_defaults_offset_to_zero() -> None:
    client = MagicMock()
    client.list_tasks = AsyncMock(return_value={"tasks": []})

    await list_tasks.run(client, envelope=None, data={})
